...
st.write("Enter the tumor feature values below to test the neural network model.")

sample_values = dict(zip(feature_names, data.data[0]))

if st.button("Use Sample Data for NN"):
    for feature in feature_names:
        st.session_state[f"nn_{feature}"] = float(sample_values[feature])
...
for i, feature in enumerate(feature_names):
    ...
    with target_col:
        input_data[feature] = st.number_input(
            feature,
            value=float(st.session_state.get(f"nn_{feature}", 0.0)),
            format="%.6f",
